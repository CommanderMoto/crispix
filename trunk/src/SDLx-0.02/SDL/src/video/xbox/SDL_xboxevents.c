...
#ifdef SAVE_RCSID
static char rcsid =
 "@(#) $Id: SDL_xboxevents.c,v 1.1 2003/08/06 04:26:19 chrisdanford Exp $";
#endif
...
void XBOX_InitOSKeymap(_THIS)
{
	...
	DIK_keymap[VK_equal] = SDLK_KP_EQUALS;
	DIK_keymap[VK_RETURN] = SDLK_KP_ENTER;
...
static void keyboard_update(void)
{
	...
	if ( g_keyboardStroke.Flags & XINPUT_DEBUG_KEYSTROKE_FLAG_KEYUP ) {
		SDL_PrivateKeyboard(SDL_RELEASED, TranslateKey(g_keyboardStroke.VirtualKey,&keysym, 1));
	} else {
		SDL_PrivateKeyboard(SDL_PRESSED,  TranslateKey(g_keyboardStroke.VirtualKey,&keysym, 0));
	}
#endif
}
